...
const RegistrationCard = () => {
  ...
  // ✅ FUNGSI FORMAT TANGGAL TANPA date-fns
      ...
      >
        ...
        <div className="flex items-center gap-4 border-b border-slate-200 pb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-400">NPSN: {school.npsn} | Telp: {school.phone}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-slate-400">Tahun Ajaran</p>
            <p className="font-semibold text-slate-700">{school.academic_year}</p>
          </div>
        </div>

        {/* Title */}
        <div className="text-center py-4">
          <h2 className="text-2xl font-bold text-blue-600">KARTU PENDAFTARAN</h2>
          <p className="text-sm text-slate-500">Sistem Penerimaan Murid Baru</p>
        </div>
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            {qrCodeDataUrl && (
              <div className="mt-2">
                ...
                <p className="text-[10px] text-slate-400 text-center mt-1">Scan untuk cek status</p>
              </div>
            )}
          </div>
          ...
          <div className="md:col-span-2 space-y-2">
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
              <div>
                ...
                <p className="font-bold font-mono text-blue-600">{application.registration_number}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium text-slate-800">{application.full_name}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400">NISN</p>
                <p className="font-medium text-slate-800">{application.nisn}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400">Jurusan Pilihan 1</p>
                <p className="font-medium text-slate-800">{application.department_1?.name || '-'}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400">Jurusan Pilihan 2</p>
                <p className="font-medium text-slate-800">{application.department_2?.name || '-'}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400">Asal Sekolah</p>
                <p className="font-medium text-slate-800">{application.school_origin?.name || '-'}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400">Tanggal Daftar</p>
                <p className="font-medium text-slate-800">{formatDate(application.registered_at)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="border-t border-slate-200 pt-4 mt-2 text-center text-xs text-slate-400">

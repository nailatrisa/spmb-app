...
const Schools = () => {
  ...
  const [formData, setFormData] = useState({ name: '', npsn: '', address: '' });
  ...
  useEffect(() => {
    ...
    setFilteredSchools(schools.filter(s => s.name.toLowerCase().includes(term) || (s.npsn && s.npsn.includes(term))));
  }, [searchTerm, schools]);

  const openForm = (school = null) => {
    if (school) {
      setEditingSchool(school);
      setFormData({ name: school.name || '', npsn: school.npsn || '', address: school.address || '' });
    } else {
